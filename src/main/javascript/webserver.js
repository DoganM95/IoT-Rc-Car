...
let servoLeft = new pigpio(18, {mode: pigpio.OUTPUT});
let servoRight = new pigpio(18, {mode: pigpio.OUTPUT});
//Future
...
const steering = {
  ...
  writeServos (rotation){
    Object.values(this).forEach(servo => {
      if(rotation > 2500){
...
//METHODS
Array.prototype.writeMotors = function(speed){
  this.forEach(motor => {
    if(speed > 100){
      motor.pwmWrite(100);
    }
...
Array.prototype.writeServos = function(steering){
  this.forEach(servo => {
    if(steering > 2500){
      servo.servoWrite(2500);
    }
